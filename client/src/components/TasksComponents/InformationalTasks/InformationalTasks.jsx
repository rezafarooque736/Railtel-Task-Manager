...
import ShowInformationalTask from './ShowInformationalTask';
import { getApiData } from '../../../Services/fetchApiFromBackend';
import { GoToButton, HeadingWithSearch } from '../TaskUtils';

const InformationalTasks = () => {
  const [InformationalTasks, setInformationalTasks] = useState([]);
  const [searchText, setsearchText] = useState('');

  const apiFetchFunc = async () => {
    const url = '/api/tasks/informational';
    const apiData = await getApiData(url);
    setInformationalTasks(apiData.data);
  };
...
export default InformationalTasks;
